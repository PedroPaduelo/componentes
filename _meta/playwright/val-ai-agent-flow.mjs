// Validador da composição ai-agent-flow (orquestrador de agentes com React Flow).
...
async function inspect(page) {
  return page.evaluate(() => {
    const root = document.querySelector("[data-slot='ai-agent-flow']")
    ...
    const firstNode = document.querySelector("[data-slot='agent-node']")
...
async function run() {
  ...
  try {
    for (const theme of ["light", "dark"]) {
      ...
      report[theme].nodeBorder = await page.evaluate(() => {
        const n = document.querySelector("[data-slot='agent-node']")
        if (!n) return null
  ...
  saveJSON("ai-agent-flow/report", report)
