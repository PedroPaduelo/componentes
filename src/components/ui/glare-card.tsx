...
export interface GlareCardProps
  ...
  /** Conteúdo (imagem, texto, ícone) renderizado dentro do card escuro. */
  children?: React.ReactNode
  /** Classes utilitárias aplicadas ao container interno que envolve os children. */
  className?: string
}

function GlareCard({ children, className, ...props }: GlareCardProps) {
  ...
  // Cleanup do setTimeout ao desmontar (evita warning de "state update on unmounted")
  React.useEffect(() => {
    return () => {
  ...
  return (
    <div
      ...
      className="relative isolate [aspect-ratio:17/21] w-full max-w-xs transition-transform delay-[var(--delay)] duration-[var(--duration)] ease-[var(--easing)] will-change-transform [contain:layout_style] [perspective:600px]"
      ref={refElement}
      onPointerMove={(event) => {
